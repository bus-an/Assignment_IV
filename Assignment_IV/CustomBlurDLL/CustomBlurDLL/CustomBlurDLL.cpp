...
BOOL ImageBlur(const ImageObject * src, ImageObject * dst, const int kernelSize)
{
    ...
    // 패딩 계산 (opencv : BORDER_REFLECT_101 방식)
    int pad_top = anchor;
    int pad_left = anchor;
    ...
    // 적분 이미지는 패딩된 이미지의 각 픽셀까지의 누적합을 저장함
    ...
    // 적분 이미지 계산: integral[i+1][j+1] = padded[i][j] + integral[i][j+1] + integral[i+1][j] - integral[i][j] >> 4번만 계산하면 됨
    for (int i = 0; i < paddedHeight; i++)
    {
        long long rowSum = 0;
        for (int j = 0; j < paddedWidth; j++)
